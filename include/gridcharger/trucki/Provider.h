...
public:
    ...
    bool getAutoPowerStatus() const final {
        return _requestedPowerAc > 0.0f
            || _dataCurrent.get<DataPointLabel::AcPowerSetpoint>().value_or(0) > 0.0f
            || _dataCurrent.get<DataPointLabel::DcPower>().value_or(0) > 0.0f;
    }

    // TODO(andreasboehm): this is huawei specific and needs to go
    void setProduction(bool enable) final {;}
    void setParameter(float val, Huawei::HardwareInterface::Setting setting) final {;}
